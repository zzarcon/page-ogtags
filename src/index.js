...
const getMetaTags = async(page) => {
  const metaTags = await page.evaluate(() => {
    const getPropertyNames = (name: string): Array < string > => {
      const normalizedName = name.replace('og:', '');

      return normalizedName.split(':');
    };
    const getPropertyPath = (names: Array < string > , value, accum = {}) => {
...
const getMeta = async(siteUrl: string) => {
...
const getMetaFromHtml = async(html: string) => {
